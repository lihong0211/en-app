...
在 [Releases](../../releases) 下载对应芯片的 dmg：芯片为 Apple M 系列选带 `-arm64` 后缀的包，Intel 芯片选不带后缀的包（Apple 菜单 →「关于本机」可查看芯片型号）。

安装包尚未经过苹果公证，首次打开时系统可能提示"Apple 无法验证""此软件需要更新"，属于正常现象，按下面任一方法处理即可：

- **右键打开**：不要双击，改为右键（或按住 Control）点击 App 图标 → 选择「打开」→ 弹窗中再次选择「打开」，之后即可正常双击使用
