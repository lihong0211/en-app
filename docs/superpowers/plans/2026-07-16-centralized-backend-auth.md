...
**Interfaces:**
- Consumes: `UserModel`（Task 2）、`get_db`（`db.py` 已有）
- Produces: `LoginInfoException`（`core/exception.py`，401 异常）、`get_current_user(authorization: str = Header(None), db: Session = Depends(get_db)) -> UserModel`（`core/auth.py`），后续 `/auth/me` 和 Phase 2 的接口鉴权都复用它
...
- [ ] **Step 3: 在 `main.py` 注册异常处理器，让 `LoginInfoException` 也走跟其它接口一样的 `{code, msg, data}` 返回格式（而不是 FastAPI 默认的 HTTP 500/详情格式）**

编辑 `backend/main.py`，在 `app = FastAPI(...)` 之后、`app.add_middleware(...)` 之前插入：
...
（放在文件顶部 `from api import users, words` 附近）并在 `register_api()` 定义之前加上：
...
（这里提前 `import auth` 是为 Task 6 铺路，Task 6 完成前这一步会导入失败，属于预期中的临时状态——下一步的测试就是验证这一点）

- [ ] **Step 4: 写测试验证 `get_current_user` 的三种分支（此时先直接测依赖函数本身，不经过还不存在的 `/auth` 路由）**
...
- [ ] **Step 5: 跑测试，此时预期失败（因为 `main.py` import 了还不存在的 `api.auth`）**

```bash
cd backend && ./.venv/bin/pytest tests/test_auth_dependency.py -v
```
Expected: FAIL，报 `ModuleNotFoundError: No module named 'api.auth'`——这是预期的中间状态，下一个 Task 就会创建这个模块

- [ ] **Step 6: 提交（先不 push，这一步 main.py 处于"引用了还不存在的模块"的中间状态，跟 Task 6 是同一个逻辑改动的两半，正常提交历史里这种情况可以接受，因为下一个 commit 紧接着补上）**

```bash
git add backend/core/exception.py backend/core/auth.py backend/main.py backend/tests/test_auth_dependency.py
git commit -m "feat: 鉴权基础设施 - LoginInfoException 与 get_current_user 依赖"
```

---

### Task 6: `/auth` 路由（微信登录、账号注册/登录、`/auth/me`）
...
**Interfaces:**
- Consumes: `hash_password`/`verify_password`/`generate_token`（Task 3）、`exchange_code_for_openid`/`fetch_wechat_userinfo`（Task 4）、`get_current_user`（Task 5）
- Produces: `POST /auth/wechat/login`、`POST /auth/register`、`POST /auth/login`、`GET /auth/me`，前三个成功时返回 `{code:200, data:{token, user}}`，`user` 是 `UserPublic` 结构
...
```
Expected: FAIL，报 `ModuleNotFoundError: No module named 'api.auth'`
...
```
Expected: 全部测试 PASS（包括之前 Task 1-5 写的测试，现在 `api.auth` 存在了，Task 5 里那个曾经预期失败的 `test_auth_dependency.py` 现在也应该 PASS）

- [ ] **Step 5: 提交**

```bash
git add backend/api/auth.py backend/tests/test_auth_api.py
git commit -m "feat: /auth 路由 - 微信扫码登录、账号注册/登录、获取当前用户"
```

---

### Task 7: 修复 `/users/*` 接口的密码泄露问题
...
- Consumes: `window.electronAPI.getToken()` / `window.electronAPI.clearToken()` / `window.electronAPI.sessionExpired()`（Task 11 会在 preload 里加上这几个方法——本任务先照接口约定写，Task 11 完成前这几个调用在真实 Electron 环境外跑不起来，属于正常的任务间依赖顺序）
...
- [ ] **Step 4: 验证能正常安装依赖（这一步只是确认 package.json 语法没写错，真正联调要等 Task 11/12 完成）**
...
### Task 10: Login.vue 组件

**Files:**
- Create: `src/render/src/components/Login.vue`

**Interfaces:**
- Consumes: `http`（Task 9）、`window.electronAPI.wechatLogin()` / `setToken()` / `completeLogin()`（Task 11）
...
### Task 12: main.js — token 本地存储、登录窗口、微信登录拦截、启动流程改造

**Files:**
- Modify: `src/main/main.js`

**Interfaces:**
- Consumes: `preload.cjs`（Task 11 约定的 IPC 频道名）
...
先在本地 MySQL 里注册一个测试账号（复用 Task 6 写好的接口，用 curl 直接调本地后端，不用等前端页面）：
...
### Task 13: App.vue 按视图切换，Desk.vue 改用共享 axios 实例
...
**Interfaces:**
- Consumes: `Login`（Task 10）、`http`（Task 9）
